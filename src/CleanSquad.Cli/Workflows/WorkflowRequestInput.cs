...
/// </summary>
/// <remarks>
///     Inline request text is materialized to a temporary markdown file so the existing workflow runtime can remain
///     path-based.
/// </remarks>
internal sealed class WorkflowRequestInput : IDisposable
{
    ...
    public void Dispose()
    {
        if (string.IsNullOrWhiteSpace(temporaryDirectoryPath) || !Directory.Exists(temporaryDirectoryPath))
        {
            return;
        }
        ...
        {
            Debug.WriteLine($"Failed to delete temporary workflow request directory '{temporaryDirectoryPath}'.");
        }
